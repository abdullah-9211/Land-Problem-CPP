...
int max_revenue_helper(unordered_map<int, long> prices, long total_area, unordered_map<int, long> table)
{
    double revenue;
    if (table[total_area] >= 0)
        return table[total_area];
    if (total_area == 0)
        revenue = 0;
    else
    {
        revenue = -numeric_limits<double>::infinity();
        for (unordered_map<int, long>::iterator i = prices.begin(); i->first <= total_area; i++)
        {
            revenue = max<long>(revenue, i->second + max_revenue_helper(prices, total_area - i->first, table));
        }
    }
    table[total_area] = revenue;
    return revenue;
}
...
int max_revenue(unordered_map<int, long> prices, long total_area)
{
    ...
    for (int i = 100; i <= total_area; i += 100)
    {
        table[i] = -numeric_limits<double>::infinity();
    }
    return max_revenue_helper(prices, total_area, table);
}
